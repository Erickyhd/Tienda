...
productos = JSON.stringify(productos);
    ...
    $.post('./modulos/ventas/realizar_venta.php', {
        ...
        try {
            ...
            respuesta = {
                venta_ok: false,
                mensaje_venta: "Error inesperado del servidor. La respuesta no es válida."
            };
        }

        if (respuesta && respuesta.venta_ok === true) {
            new PNotify({
                title: 'Éxito',
            ...
            if (respuesta.impresion_ok === false) {
                new PNotify({
                    title: 'Advertencia de impresión',
                    text: respuesta.mensaje_impresion,
                    type: 'warning'
                });
            }
            ...
            new PNotify({
                ...
                text: respuesta.mensaje_venta || 'Hubo un error al procesar la venta. Por favor, revisa la configuración de la impresora o contacta al administrador.',
                type: 'error'
            });
...
function escuchar_elementos() {
    ...
    $("#pago_usuario").keyup(function (evento) {
        ...
        if (cambio >= 0 && !isNaN(pago)) {
            $("#contenedor_cambio").text("S/" + cambio).parent().show();
        } else {
            $("#contenedor_cambio").parent().hide();
        }
