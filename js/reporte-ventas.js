...
function dibuja_tabla_ventas_por_familia(ventas) {
    ...
    ventas.forEach(function (a) {
            ...
            var invertido = parseFloat(a.total) - parseFloat(a.utilidad);
            invertido_por_familia += invertido;
    ...
    $contenedorReportePorFamilia
        .append(
            ...
            '</td>' +
            '<td> S/' +
            invertido_por_familia.toFixed(2) +
            '</td>' +
            '<td>' +
            '<strong>S/' +
...
function dibuja_tabla_ventas(ventas) {
    $("#mostrar_total, #mostrar_invertido, #mostrar_utilidad").text("").parent().hide();
    ...
    for (var i = ventas_totales.length - 1; i >= 0; i--) {
        ...
        var invertido = parseFloat(ventas_totales[i].total) - parseFloat(ventas_totales[i].utilidad);
        ayudante_invertido += invertido;
    ...
    $("#mostrar_invertido").text(ayudante_invertido.toFixed(2)).parent().show();
