...
function hacer_venta($productos, $total, $ticket, $cambio)
{
    ...
    foreach ($productos as $producto) {
        ...
        $todo_correcto = $todo_correcto && quitar_piezas($producto->cantidad, $producto->rowid);
        ...
        $todo_correcto = $todo_correcto && $resultado_sentencia;
    }

    // Ingresar dinero en caja
    $todo_correcto = $todo_correcto && ingresar_dinero_venta_caja($total, $numero_venta);

    if ($todo_correcto) {
        $base_de_datos->commit();
        
        $resultado_impresion = true; // Asumir éxito por defecto
        if ($ticket === true) {
            require_once RAIZ . "/modulos/ticket.php";
            $resultado_impresion = imprime_ticket($productos, $numero_venta, $cambio);
        }
        return array(
            "venta_ok" => true,
            "impresion_ok" => $resultado_impresion === true,
            "mensaje_impresion" => $resultado_impresion === true ? "Ticket impreso correctamente" : $resultado_impresion
        );

    } else {
        $base_de_datos->rollBack();
        return array("venta_ok" => false, "mensaje_venta" => "Error al registrar la venta.");
    }
}
